...
PRO reading, colArray
  ...
  readcol, 'two.txt', v1, v2, v3, format = 'A,A,A', delim = ','  ; read stuff from file, store into vars v1, v2, & v3
  colArray = [ [v1],[v2],[v3] ]                                  ; array-inize vars indexed by [row, column]
  RETURN
END
...
FUNCTION wordsmash, col1, col2, ans                       ; take answers and smoosh them
  pNum = SIZE(ans, /N_ELEMENTS) - 1                       ; number of hilarious lines
  smash = MAKE_ARRAY(pNum + 1, 3, /STRING)
  smash[*,0] = col1                                       ; put columns in smash array
  smash[*,1] = col2
  smash[*,2] = ans
  RETURN, smash
END
...
PRO regurgitate, final
  ...
  FOR i = 0, pNum, 1 DO BEGIN
     print, final[i,0], ' ', final[i,2], final[i,1]      ; deliver punchline
  ENDFOR
END
