pro main
    ...
    ;INPUT: (None)
    ;OUTPUT: (None)
    img = mrdfits('idl_image.fits')
    imghdr= headfits('idl_image.fits')
...
function colorzoom, pic
	...
	print 'use themouse to select a kitten'
        cursor,x,y
	...
	if (x+150 GT n_elements(pic[*,0)) or (x-150 LT 0) or (y+150 GT n_elements(pic[0,*]) or(y-150 LT 0) then begin
		print, 'OUT OF BOUNDS'
	endif else begin
		new_pic=img(x-150:x+150, y-150:y+150)
...
function better_half, pic
  ...
  ;OUTPUT: the color-altered image
	cathead = colorzoom(pic)
	cathead(2./3.*n_elements(cathead[*,0]):n_elements(cathead[*,0])-1,*)=cathead(2./3.*n_elements(cathead[*,0]):n_elements(cathead[*,0])-1,*)*1.33 
